...
int main(){
    ...
    unsigned char* data = stbi_load("/home/crazi_grace/MINE/Codezzz/OpenGL/resources/container.jpg", &width, &height, &nrChannels, 0);
    if (data){
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    data = stbi_load("/home/crazi_grace/MINE/Codezzz/OpenGL/resources/awesomeface.png", &width, &height, &nrChannels, 0);
    if (data) {
    ...
    while (!glfwWindowShouldClose(window)){
        ...
        for (unsigned int i=0; i<10; i++){
            // model matrix for each object and pass it to shader
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    camera.processMouseMovement(static_cast<float>(yoffset));
}
